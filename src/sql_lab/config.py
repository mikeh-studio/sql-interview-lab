...
PRIMARY_ENV_PREFIX = "SQL_LAB_"
LEGACY_ENV_PREFIX = "DATA_INTERVIEW_LAB_"


def _environment_value(name: str, default: str | None = None) -> str | None:
    """Read a SQL Lab setting while preserving its former Data Lab name."""
    configured = os.getenv(name)
    if configured is not None:
        return configured
    compatibility_name = name.replace(PRIMARY_ENV_PREFIX, LEGACY_ENV_PREFIX, 1)
    return os.getenv(compatibility_name, default)


def default_history_db_path() -> Path:
    configured = _environment_value("SQL_LAB_HISTORY_DB")
    if configured:
        return Path(configured).expanduser()
    current_path = Path.home() / ".sql-interview-lab" / "history.db"
    compatibility_path = Path.home() / ".data-interview-lab" / "history.db"
    if not current_path.exists() and compatibility_path.exists():
        return compatibility_path
    return current_path


def history_limit_from_env() -> int:
    limit = int(_environment_value("SQL_LAB_HISTORY_LIMIT", "200"))
    if limit < 1:
...
class Settings:
    ...
    def from_env(cls) -> "Settings":
        timeout = float(_environment_value("SQL_LAB_LLM_TIMEOUT", "600"))
        if timeout <= 0:
            raise ValueError("SQL_LAB_LLM_TIMEOUT must be positive")
        advanced_timeout = float(
            _environment_value("SQL_LAB_ADVANCED_LLM_TIMEOUT", "1200")
        )
        if advanced_timeout <= 0:
            raise ValueError("SQL_LAB_ADVANCED_LLM_TIMEOUT must be positive")
        return cls(
            llm_provider=_environment_value("SQL_LAB_LLM_PROVIDER", "codex"),
