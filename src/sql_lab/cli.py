...
from sql_lab.engines.factory import SUPPORTED_DIALECTS, create_engine, execution_mode
from sql_lab.exercises import get_static_exercise
from sql_lab.generation import ExerciseGenerationError
from sql_lab.grading.grader import GradeResult, Grader, ReferenceSolutionError
...
def main(
    ...
    static: Annotated[
        bool,
        typer.Option("--static", help="Use the bundled offline Phase 1 exercise."),
    ] = False,
    ...
    try:
        ...
        console.print(
            f"[bold red]Could not start SQL Lab:[/bold red] {escape(str(exc))}"
        )
        if not static:
