...
from data_interview_lab.engines.base import QueryResult, SQLExecutionError
...
from data_interview_lab.grading.grader import (
    GradeResult,
    Grader,
    ReferenceSolutionError,
)
from data_interview_lab.llm import LLMProviderError
from data_interview_lab.models import Dialect, Difficulty, Exercise, ExerciseRequest
from data_interview_lab.services import generate_exercise
...
def _data_interview_lab_is_running(url: str) -> bool:
    try:
...
def _launch_web(host: str, port: int, *, open_browser: bool) -> None:
    ...
    if _data_interview_lab_is_running(url):
        console.print(
            f"[bold green]Data Interview Lab[/bold green] is already running at {url}"
        )
        if open_browser:
    ...
    if _port_is_in_use(host, port):
        raise ValueError(
            ...
            f"Try `data-interview-lab --web --port {port + 1}`."
        )

    console.print(f"[bold green]Data Interview Lab[/bold green] is running at {url}")
    ...
    if open_browser:
        ...
        timer.start()
    uvicorn.run(
        "data_interview_lab.web.app:app", host=host, port=port, log_level="info"
    )
    ...
    try:
        while True:
            try:
                ...
                console.print("\n[dim]Leaving Data Interview Lab.[/dim]")
                return "quit"
...
def main(
    ...
    try:
        ...
        console.print(
            f"[bold red]Could not start Data Interview Lab:[/bold red] {escape(str(exc))}"
        )
        if not static:
            console.print(
                "[dim]Try `data-interview-lab --static` for the offline exercise.[/dim]"
            )
